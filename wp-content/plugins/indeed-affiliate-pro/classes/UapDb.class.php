	...
	class UapDb
	{
				...
				if (!empty($post_data['id'])) {
					...
					if (!empty($data)) {
						...
						;",
							$post_data['type'],
							$post_data['rank_id'],
							stripslashes_deep($post_data['subject']),
							stripslashes_deep($post_data['message']),
							stripslashes_deep($post_data['pushover_message']),
							$post_data['pushover_status'],
							$post_data['status'],
							$post_data['id']
						);
						$wpdb->query($q);
		...
		public function return_settings_from_wp_option($type = '', $only_name = false, $return_default = false)
		{
			...
			switch ($type) {
				...
				case 'account_page':
					$arr = array(
						'uap_ap_top_theme' => 'uap-ap-top-theme-2',
...
<div><strong>Refuse Referrals :</strong> {refuse_referrals}</div>
				...
				if (!empty($post_data['id'])) {
					...
					if ($exists) {
						...
						;",
							...
							$post_data['bonus'],
							$post_data['pay_per_click'],
							$post_data['cpm_commission'],
							...
							stripslashes_deep($post_data['achieve']),
		...
		public function getReferralsReports($affiliate_id = 0)
		{
			...
			if ($affiliate_id) {
				...
				$q = $wpdb->prepare(
					"SELECT COUNT(id) as count_referrals FROM $table WHERE affiliate_id=%d AND status=2;",
					$affiliate_id
				);
				$temp = $wpdb->get_row($q);
				if ($temp && isset($temp->count_referrals)) {
		...
		public function get_referral_report_by_date($affiliate_id = 0, $start_time = '', $end_time = '')
		{
			...
			if ($affiliate_id) {
				...
				$q = $wpdb->prepare(
					"SELECT COUNT(id) as count_referrals FROM $table WHERE affiliate_id=%d AND date>%s AND date<%s AND status=2;",
					$affiliate_id,
					$start_time,
					$end_time
				);
		...
		public function save_referral($post_data = array())
		{
			...
			if (!empty($post_data)) {
				...
				$q = $wpdb->prepare(
					"INSERT INTO $table VALUES
					( NULL, %d, %s, %d, %d, %s, %s, %s, %s, %d, %d, %s, %s, %s, %s, %s );",
					$post_data['refferal_wp_uid'],
					$post_data['campaign'],
					$post_data['affiliate_id'],
					$post_data['visit_id'],
				...
				$log->warning('post_data', $post_data);

				// if ($post_data['source'] == 'woo') {
				// 	$q = $wpdb->prepare(
				// 		"INSERT INTO $table VALUES
				// 		( NULL, %d, %s, %d, %d, %s, %s, %s, %s, %d, %d, %s, %s, %s, %s, %s );",
				// 			$post_data['refferal_wp_uid'],
		...
		public function change_referral_status($id = 0, $new_status = 0, $search_parent = TRUE, $search_child = TRUE)
		{
			...
			var_dump('change_referral_status');
			echo "<br>";
			...
			if (get_option('uap_mlm_enable')) {
				var_dump('uap_mlm_enable');
				echo "<br>";
				if ($search_parent) {
					var_dump('search_parent');
					echo "<br>";
		...
		public function get_referral_id_for_reference($reference = '', $source = '')
		{
			 ...
			 * @return int
			 */
			var_dump('get_referral_id_for_reference');
			echo "<br>";
			if ($reference) {
				global $wpdb;
				$table = $wpdb->prefix . "uap_referrals";
		...
		public function referralIdLast($visit_id=0)
		{
			if ($visit_id) {
				global $wpdb;
				$table = $wpdb->prefix . 'uap_referrals';

				$query = "SELECT max(id) as id FROM $table WHERE reference = %s ";
				global $log;
				$log->warning('query', [$query, "mlm_$visit_id"]);
				
				$q = $wpdb->prepare($query, "mlm_$visit_id");
				$data = $wpdb->get_row($q);
		...
		public function update_referal_id($visit_id=0, $parent_referral_id=0)
		{
			if ($visit_id) {
				global $wpdb;
				$table = $wpdb->prefix . 'uap_referrals';
				$q = $wpdb->prepare("SELECT id  FROM $table WHERE id=%d ", $visit_id);
				$exists = $wpdb->get_row($q);
				if ($exists) {
					$q = $wpdb->prepare("UPDATE $table
										SET parent_referral_id=%d
										WHERE id=%d ", $parent_referral_id, $visit_id);
					$wpdb->query($q);
				}
		...
		public function save_landing_commission($post_data = array())
		{
			...
			if (empty($post_data['id'])) {
				...
				$q = $wpdb->prepare(
					"UPDATE $table SET slug=%s, settings=%s, status=%s WHERE id=%d ",
					$post_data['slug'],
					$settings,
					$post_data['status'],
					$post_data['id']
		...
		public function save_coupon_affiliate_pair($post_data = array())
		{
			...
			if (empty($post_data['id'])) {
				...
				$q = $wpdb->prepare(
					"INSERT INTO $table VALUES(null, %s, %s, %s, %s, %s);",
					$post_data['code'],
					$post_data['affiliate_id'],
					$post_data['type'],
					$settings,
				...
				$q = $wpdb->prepare(
					"UPDATE $table SET code=%s, settings=%s, status=%s, type=%s WHERE id=%d ",
					$post_data['code'],
					$settings,
					$post_data['status'],
					$post_data['type'],
		...
		public function doesCouponExists($type = '', $search = '')
		{
			...
			switch ($type) {
				case 'woo':
					...
					$query = $wpdb->prepare($strquery, $search);
		...
		public function create_wallet_item($type = '', $referral_list = array(), $affiliate_id = 0)
		{
			...
			if (is_array($referral_list)) {
				foreach ($referral_list as $temporaryId => $referralId) {
					...
					if (
						!isset($temporary['affiliate_id']) || $temporary['affiliate_id'] != $affiliate_id
