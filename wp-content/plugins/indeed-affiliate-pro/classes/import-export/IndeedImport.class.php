		...
		switch ($entity_name){
			...
			case 'usermeta':
					foreach ($xml_object->$entity_name->children() as $meta_key=>$object){
						if ( !$indeed_db->does_usermeta_exists($object->user_id, $object->meta_key ) ){
							/// post meta does not exists
							$indeed_db->custom_insert_usermeta($object->user_id, $object->meta_key, $object->meta_value);
						}
					}
				break;
			case 'indeed_wp_capabilities':
					...
					foreach ($xml_object->$entity_name->children() as $object){
						if ( !$indeed_db->does_usermeta_exists( $object->user_id, $meta_key ) ){
							/// post meta does not exists
							$indeed_db->custom_insert_usermeta($object->user_id, $meta_key, $object->meta_value);
						}
					}
				break;
			default:
