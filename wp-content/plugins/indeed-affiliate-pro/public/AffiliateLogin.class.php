<?php
if (!class_exists('AffiliateLogin')){
	class AffiliateLogin{
		...
		public function print_login_form($args=array(), $user_role='unreg', $affiliate_id=0){
			...
			if (!empty($_GET['uap_success_login'])){
				...
				if (isset($arr['uap_login_error_email_pending']) && $arr['uap_login_error_email_pending']){
					...
					$login_faild = esc_html__('Error', 'uap');
				}
				...
				if (!$login_faild){
					$login_faild = esc_html__('Captcha Error', 'uap');
				}
		...
		public function uap_check_email_verification_status($uid = 0, $redirect_url=''){
			/*
