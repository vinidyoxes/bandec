...
class Referral_Main{
	...
	public function save_referral_unverified($args=array()){ // protected
		...
		if ($referral_id){
			...
			$lastId = $indeed_db->referralIdLast($referral_id);
			$args2 = $args;
			$args2['amount']			= 10;
			$args2['source']			= "mlm";
			$args2['reference']			= "mlm_$referral_id";
			$args2['child_referral_id']	= $lastId>0 ? $lastId : $referral_id;
			$referral_id2 				= $indeed_db->save_referral($args2);
			// if($lastId>0){
			// 	$args2['child_referral_id']	= $lastId;
			// 	$referral_id2 				= $indeed_db->save_referral($args2);
			// }else{
			// }
			
			if ($referral_id2){
				$indeed_db->update_referal_id($lastId>0 ? $lastId : $referral_id, $referral_id2);
				
				$lastId = $indeed_db->referralIdLast($referral_id);
				$args2['affiliate_id']		= $indeed_db->affiliate_get_id_by_uid($args['refferal_wp_uid']);
				$args2['child_referral_id']	= $lastId;
				$referral_id3 				= $indeed_db->save_referral($args2);
				if ($referral_id3) $indeed_db->update_referal_id($lastId, $referral_id3);
			}
			// else{
			// 	$indeed_db->update_referal_id($referral_id, $referral_id2);
				
			// 	$lastId = $indeed_db->referralIdLast($referral_id);
			// 	$args2['affiliate_id']		= $indeed_db->affiliate_get_id_by_uid($args['refferal_wp_uid']);
			// 	$args2['child_referral_id']	= $referral_id;
			// 	$referral_id3 				= $indeed_db->save_referral($args2);
			// 	if ($referral_id3) $indeed_db->update_referal_id($lastId, $referral_id3);
			// }
		}
		// $log->warning('args2', $args2);
