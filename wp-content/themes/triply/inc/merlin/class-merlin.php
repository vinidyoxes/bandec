...
//		add_action( 'after_switch_theme', array( $this, 'switch_theme' ) );
	...
	function _ajax_plugins() {
		...
		if ( $json ) {
			$this->logger->debug(
				__( 'A plugin with the following data will be processed', 'triply' ),
				array(
			...
			$this->logger->debug(
				__( 'A plugin with the following data was processed', 'triply' ),
				array(
					'plugin_slug' => $_POST['slug'],
				)
			);
	...
	function _ajax_content() {
		...
		if ( isset( $_POST['proceed'] ) ) {
			if ( is_callable( $this_content['install_callback'] ) ) {
				$this->logger->info(
					__( 'The content import AJAX call will be executed with this import data', 'triply' ),
					array(
		...
		if ( $json ) {
			...
			$this->logger->error(
				__( 'The content import AJAX call failed with this passed data', 'triply' ),
				array(
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['options'] ) ) {
			$content['options'] = array(
				'title'            => esc_html__( 'Options', 'triply' ),
				'description'      => esc_html__( 'Sample theme options data.', 'triply' ),
				'pending'          => esc_html__( 'Pending', 'triply' ),
				'installing'       => esc_html__( 'Installing', 'triply' ),
				'success'          => esc_html__( 'Success', 'triply' ),
