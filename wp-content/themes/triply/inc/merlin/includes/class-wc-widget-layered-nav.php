...
class Triply_Widget_Layered_Nav extends WC_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'woocommerce triply_widget_layered_nav woocommerce-widget-layered-nav';
        $this->widget_description = esc_html__('Display a list of attributes to filter products in your store.(Triply custom)', 'triply');
        $this->widget_id          = 'triply_woocommerce_layered_nav';
        $this->widget_name        = esc_html__('Triply Filter Products by Attribute', 'triply');
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            'title'      => array(
                'type'  => 'text',
                'std'   => esc_html__('Filter by', 'triply'),
                'label' => esc_html__('Title', 'triply'),
            ),
            'attribute'  => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => esc_html__('Attribute', 'triply'),
                'options' => $attribute_array,
            ),
            'query_type' => array(
                'type'    => 'select',
                'std'     => 'and',
                'label'   => esc_html__('Query type', 'triply'),
                'options' => array(
                    'and' => esc_html__('AND', 'triply'),
                    'or'  => esc_html__('OR', 'triply'),
                ),
            ),
        );
    }
    ...
    protected function layered_nav_list($terms, $taxonomy, $query_type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($count > 0 || $option_is_set) {
                ...
                $term_html = '<a rel="nofollow" class="triply-button-type" href="' . $link . '">' . esc_html($term->name) . '</a>';
                    ...
                    if (isset($fields[0]["type"])) {
                        ...
                        switch ($fields[0]["type"]) {
                            case 'color':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="triply-color-type" href="' . esc_url($link) . '">';
                            ...
                            case 'image':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="triply-image-type" href="' . esc_url($link) . '">';
